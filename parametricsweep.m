function [para,master,envmaster,count1,count2,R1,R2,Re3,Re4,Rb7,Rb8,Rm9,Rm10,x,envrow,bmatcol,sides,delta_t,transteps,basestatus,bmat,bmatmaster,NL,count3,matlist,matpropsmaster,matpropsrow,matparatype,tranmin]...
...
if count1==lp                                                               % if the all parameters have been considered the nested loop will break
    ...
    if isnan(str2double(para{count1,2}(1,end)))==0                              % if there is a feature number at the end of the parameter
        if strcmp(para{count1,2}(1,end-1),'.')==0
            ...
            tt=find(x(layer,9)==featnum);
            if rownum > size(x,1)
                errordlg('Layer for parameterized feature does not exist')
            return
            end
            layers=layer(1,1)+tt-1';
    ...
    for i=1:n
        % Transient
        %Transient here because no matrix needs duplication to assign
        %transient parameters. Break included because entire matrix
        %assigned in one loop.
        if strcmp(para{count1,1},'Transient')==1
            tranhold=para(count1,4:6);
            [tranmin,transteps,tranmax]=tranhold{:};
            %         delta_t=(tranmax-tranmin)/transteps;
            range=linspace(tranmin,tranmax,transteps+1);
