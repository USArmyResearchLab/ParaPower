...
for timestep = 1:length(time)
        ...
        for Xii=1:NRx
            for Yjj=1:NCy
                if  ckMatl(Xii,Yjj,Zkk) == 0 || Melt(Xii,Yjj,Zkk) > 0
                    stressx(Xii,Yjj,Zkk,timestep) = NaN;
                    stressy(Xii,Yjj,Zkk,timestep) = NaN;
                    stressz(Xii,Yjj,Zkk,timestep) = NaN;
                else
                    stressx(Xii,Yjj,Zkk,timestep) = (EX(Mats(Xii,Yjj,Zkk))/(1-nuX(Mats(Xii,Yjj,Zkk))))*((Lfx(Yjj)/(dx(Xii)*(cteX(Mats(Xii,Yjj,Zkk))*delT(Xii,Yjj,Zkk)+1)))-1);
                    stressy(Xii,Yjj,Zkk,timestep) = (EY(Mats(Xii,Yjj,Zkk))/(1-nuY(Mats(Xii,Yjj,Zkk))))*((Lfy(Xii)/(dy(Yjj)*(cteY(Mats(Xii,Yjj,Zkk))*delT(Xii,Yjj,Zkk)+1)))-1);
                    stressz(Xii,Yjj,Zkk,timestep) = (EZ(Mats(Xii,Yjj,Zkk))/(1-nuZ(Mats(Xii,Yjj,Zkk))))*((Lfz(Zkk)/(dz(Zkk)*(cteZ(Mats(Xii,Yjj,Zkk))*delT(Xii,Yjj,Zkk)+1)))-1);
                end
            end
        end
    end
    ...
    for Xii=1:NRx
        for Yjj=1:NCy
            for Zkk=1:NLz
                ForceX(Xii,Yjj,Zkk,timestep) = stressx(Xii,Yjj,Zkk,timestep)*dy(Yjj)*dz(Zkk);
                ForceY(Xii,Yjj,Zkk,timestep) = stressy(Xii,Yjj,Zkk,timestep)*dx(Xii)*dz(Zkk);
                ForceZ(Xii,Yjj,Zkk,timestep) = stressz(Xii,Yjj,Zkk,timestep)*dy(Yjj)*dx(Xii);
            end
        end
    end
