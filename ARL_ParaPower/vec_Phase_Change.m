...
if any(changing)  %things are changing phase, update
    ...
    %Update DOF
    PH = PH + direction .* abs(T-Tm(Mat)).*(RHO.*CP./Lv(Mat));  %increment PH according to excess sensible
    T = T - direction .* abs(T-Tm(Mat));                %decrement T by temperature excess
    ...
    T = T + direction .* PH_ex .* (Lv(Mat)./(RHO.*CP));
end
