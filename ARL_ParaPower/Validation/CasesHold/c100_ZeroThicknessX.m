...
Features(1).x      = [0 .05];  %.x, .y & .z are the two element vectors that define the corners
...
Features(end).dx   = 1;     %These define the number of elements in each features.  While these can be 
...
Features(end).x  = [.05 .15];  %.x, .y & .z are the two element vectors that define the corners
Features(end).dx   = 2;     %These define the number of elements in each features.  While these can be 
...
Features(end+1)=Features(end);
Features(end).x    = [ 1 1]*max(Features(1).x);
Features(end).Matl = 'Air';

%Zero layer thickness at max X
Features(end+1)=Features(end);
Features(end).x    = [ 1 1]*max(Features(2).x);
Features(end).Matl = 'Air';
