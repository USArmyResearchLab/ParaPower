...
Features(1).y      = [0 .05];  %.x, .y & .z are the two element vectors that define the corners
Features(end).x    = [0 .15];  %of each features.  X=[X1 X2], Y=[Y1 Yz], Z=[Z1 Z2] is interpreted
Features(end).z    = [0 .15];   %that corner of the features are at points (X1, Y1, Z1) and (X2, Y2, Z2).
...
Features(end).dy   = 1;     %These define the number of elements in each features.  While these can be 
Features(end).dx   = 3;     %values from 2 to infinity, only odd values ensure that there is an element
Features(end).dz   = 3;    %at the center of each features
...
Features(end).y  = [.05 .15];  %.x, .y & .z are the two element vectors that define the corners
Features(end).dy   = 2;     %These define the number of elements in each features.  While these can be 
...
Features(end).y    = [ 1 1]*min(Features(1).x);
Features(end).x    = [.05 .10];
Features(end).z    = [.05 .10];
...
Features(end+1)=Features(end);
Features(end).y    = [ 1 1]*max(Features(1).y);
Features(end).Matl = 'Air';

%Zero layer thickness at max X
Features(end+1)=Features(end);
Features(end).y    = [ 1 1]*max(Features(2).y);
Features(end).Matl = 'Air';
