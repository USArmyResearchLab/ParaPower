...
Features(end).x  = [.05 .1];  %.x, .y & .z are the two element vectors that define the corners
Features(end).y  = [.075 .1];  %.x, .y & .z are the two element vectors that define the corners
...
Features(end).z  = [1 1]*min(Features(1).z);
...
%Zero layer thickness at max X, only the central element
Features(end+1)=Features(1); 
Features(end).x  = [1 1]*min(Features(1).x);
Features(end).Matl = 'Al';
Features(end).dx   = 1;
Features(end).dy   = 1;
Features(end).dz   = 1;
...
%Non-Zero layer thickness at max X, only the central element
Features(end+1)=Features(1); 
Features(end).z  = [1 1.5]*max(Features(1).z);
Features(end).Matl = 'Al';
Features(end).dx   = 1;
Features(end).dy   = 1;
Features(end).dz   = 1;
