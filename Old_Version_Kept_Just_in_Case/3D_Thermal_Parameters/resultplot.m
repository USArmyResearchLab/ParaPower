    ...
    if i>1 && solnum~=1
     ...
     if i==solnum && highlightsol==1
         ...
         if strcmp(plotname,'Transient')==1    
            ...
            for ii=1:steps
            yyaxis left
            plot(xdata(R1,1),TempMaster(R1,1),'c.','markers',tempmarkersize+scale)
            text(xdata(R1,1),TempMaster(R1,1),txt(R1,1),'FontSize',txtsize+scale/4)
            yyaxis right
            plot(xdata(R1,1),StressMaster(R1,1),'ro','markers',stressmarkersize+scale)
            text(xdata(R1,1),StressMaster(R1,1),txt(R1,1),'FontSize',txtsize+scale/4)
            pause(0.3)
            R1=R1+1;
            end
         else
