...
[ly,~]=size(ylayout);   
[ND,~]=size(idx);                                                           % determines the number of devices
...
xgrid=unique([xskinny,extrarangexskinny]);                                  % combines coor vectors and finds only unique instances
ygrid=unique([yskinny,extrarangeyskinny]);
